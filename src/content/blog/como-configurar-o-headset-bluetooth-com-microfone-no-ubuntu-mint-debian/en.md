...
<policy user="pulse"> <allow send\_destination="org.ofono"/> </policy>

5\. To make ofono work it is necessary to provide a modem for it. And for that we are going to install a modem emulator called [phonesim](https://packages.debian.org/stretch/devel/ofono-phonesim) that will be implemented by ofono to work. Install the `phone sim` :
