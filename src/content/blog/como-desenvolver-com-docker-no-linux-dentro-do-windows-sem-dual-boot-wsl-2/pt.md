...
description: O WSL é um recurso disponível no Windows 10, mediante habilitação,
  desde a versão 1607, e desde a versão 2004 a versão 2 foi disponibilizada para
  todos. O WSL permite executar comandos em um distribuição Linux diretamente no
  Windows, sem precisar de uma máquina virtual ou dual boot.
date: 2020-06-18T11:59:11.000Z
lang: pt
translationKey: como-desenvolver-com-docker-no-linux-dentro-do-windows-sem-dual-boot-wsl-2
slug: como-desenvolver-com-docker-no-linux-dentro-do-windows-sem-dual-boot-wsl-2
...
Um exemplo prático para os desenvolvedores de [nodejs e docker](http://marquesfernandes.com/desenvolvimento-otimizado-em-nodejs-com-typescript-docker-e-eslint/), é não precisar mais usar a flag `--legacy-watch` com o nodemon, as mudanças nos arquivos serão propagadas com a mesma velocidade e compatibilidade, afinal, elas estarão rodando no linux.
...
#### Reduzir o tamanho do Docker Data

O Docker Desktop para Windows, que usa WSL2, armazena todos os arquivos de imagem e contêiner em um volume virtual separado (vhdx). Este arquivo de disco rígido virtual pode crescer automaticamente quando precisa de mais espaço (até um certo limite). Infelizmente, se você recuperar algum espaço, ou seja, removendo imagens não utilizadas, o vhdx não encolhe automaticamente. Felizmente, você pode reduzir seu tamanho manualmente chamando este comando no PowerShell (como administrador):

Optimize-VHD -Path c:\\path\\to\\data.vhdx -Mode Full

O docker-desktop-data vhdx está normalmente neste local:

%LOCALAPPDATA%\\Docker\\wsl\\data\\ext4.vhdx

Caso você encontre problemas para executar o Optimize-VHD, você pode tentar a seguinte alternativa:
